...
#
# onTask:        tasks.json has entries  → load slice from board, update build-kit-dotnet/.slices/
# onPlannedSlice: build-kit-dotnet/.slices/ has a "Planned" slice → build it
#
...
#   project_dir — path to the .NET solution root; required (this is a generic
#                 template — pass it explicitly, or set DOTNET_PROJECT_DIR)

set -euo pipefail

KIT_DIR="$(cd "$(dirname "$0")" && pwd)"
ITERATIONS="${1:-0}"
PROJECT_DIR="${2:-"${DOTNET_PROJECT_DIR:-}"}"
if [[ -z "$PROJECT_DIR" ]]; then
  echo "[ralph] Missing project_dir: pass it as an argument (./ralph.sh 0 /path/to/solution) or set DOTNET_PROJECT_DIR." >&2
  exit 1
fi
...
# NOTE: this only checks build-kit-dotnet/.eventmodelers/config.json itself,
# not ancestor directories — unlike lib/ralph.js's loadLocalConfig (used by
# the JS entry points), which walks up to find the repo-root copy too. This
# script is the secondary/bash-only alternative (see README); if you rely on
# the repo-root config without one directly here, use ralph-claude.js instead.
...
# Returns 0 if any JSON under build-kit-dotnet/.slices/ contains a "Planned" status
has_planned_slices() {
