...
// Fixed vs. the original build-kit/ script: that one hardcoded '.slices' as
// the git pathspec/add-target, which only works if SLICES_DIR sits directly
// under repoPath. Here (and in the original's own nested .build-kit/.slices
// layout too, actually) it doesn't — compute the real relative path instead.
const SLICES_PATHSPEC = relative(repoPath, SLICES_DIR);
...
const server = createServer(async (req, res) => {
    ...
    if (pathname === '/api/slices' && req.method === 'GET') {
        try {
            ...
            // slice.json files
            const trackedFiles = isRepo ? gitLsTree(repoPath, revision, SLICES_PATHSPEC) : [];
