...
- [x] 3.4.1 [FIX 3.4] Fix: Wolverine TypeLoadMode.Dynamic has no IAssemblyGenerator registered
  - **Do**: Address the error: `Api.Host` crashes on startup before binding port 5132 with `System.InvalidOperationException: Wolverine is running in TypeLoadMode.Dynamic... but no IAssemblyGenerator (Roslyn) is registered`.
    1. Add `<PackageReference Include="WolverineFx.RuntimeCompilation" Version="6.22.0" />` to `src/Api.Host/Api.Host.csproj` (pin the same 6.22.0 line as the other WolverineFx.* packages already referenced there).
    2. Confirm `dotnet restore`/`dotnet build src/Api.Host/Api.Host.csproj` succeed with the new package.
    3. Re-launch `Api.Host` and confirm it now binds port 5132 and serves `/health` — this is purely an `Api.Host`-level infrastructure gap (missing runtime-codegen compiler), not specific to the SubmissionIntake module; do not touch SubmissionIntake source files for this fix.
  - **Files**: `src/Api.Host/Api.Host.csproj`
  - **Done when**: `Api.Host` starts cleanly and `curl -sf http://localhost:5132/health` returns `{"status":"Healthy"}`
  - **Verify**: `dotnet build src/Api.Host/Api.Host.csproj && echo PASS`
...
- [x] 4.1.2 [FIX 4.1] Fix: remove redundant `Api.Host` → `SubmissionIntake.Infrastructure` direct `ProjectReference`
...
- [x] 5.4.1 [FIX 5.4] Fix: cross-test-class data collision breaks `No_matching_SubmissionQueue_entry_appends_nothing` when the full IntegrationTests suite runs together
  - **Do**: Address the error: reproduced independently — `dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests` (full suite, not the scoped `--filter DetectPotentialDuplicate`) fails `No_matching_SubmissionQueue_entry_appends_nothing` deterministically (10/11 pass, this one fails every run, not flaky-intermittent). Root cause: `DetectPotentialDuplicateOnNormalizationHandlerTests.cs`'s "no-match" test and `SubmissionQueueProjectorTests.cs` (from tasks 4.7/4.7.1) seed `SubmissionQueue` rows using the same shared `SubmissionIntakePostgresFixture`/collection without resetting state between test classes, and use overlapping default field literals (`classOfBusiness`/`territory`/`namedInsured`) — so a row seeded by one test class satisfies the "match" condition in another.
    1. Give each test method's seed data unique `classOfBusiness`/`territory`/`namedInsured` values (e.g. suffix with a per-test GUID or the test's own submissionId) so no two tests across the whole `IntegrationTests` project can accidentally collide on DEC-010's exact-match fields, regardless of execution/collection order.
    2. Alternatively/additionally, if the shared fixture supports it cleanly, reset/truncate `SubmissionQueue` (and any other shared read-model tables) between test classes — but prefer fix (1) first since it's more localized and doesn't risk breaking other tests' assumptions about persisted state within their own class.
    3. Re-run the FULL suite (not a filtered subset) to confirm the fix actually holds under real cross-class execution, not just the originally-affected test in isolation.
  - **Files**: `tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests/DetectPotentialDuplicateOnNormalizationHandlerTests.cs`, `tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests/SubmissionQueueProjectorTests.cs` (whichever files need unique seed data — read both first to find every colliding literal)
  - **Done when**: `dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests` (full project, no filter) passes 100%, run at least twice to rule out order-dependent flakiness
...
- [x] 5.9 [VERIFY] Quality checkpoint: build + all module tests pass
  - **Do**: `dotnet build src/BrokerConnect.slnx && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Domain.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Api.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests`
  - **Verify**: All exit 0
  - **Done when**: No errors
  - **Commit**: `chore(submission-intake): pass quality checkpoint` (if fixes needed)
...
- [x] 6.9 [VERIFY] Quality checkpoint: build + all module tests pass
  - **Do**: `dotnet build src/BrokerConnect.slnx && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Domain.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Api.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests`
  - **Verify**: All exit 0
  - **Done when**: No errors
  - **Commit**: `chore(submission-intake): pass quality checkpoint` (if fixes needed)
...
- [x] 7.9 [VERIFY] Quality checkpoint: build + all module tests pass
  - **Do**: `dotnet build src/BrokerConnect.slnx && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Domain.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Api.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests`
  - **Verify**: All exit 0
  - **Done when**: No errors
  - **Commit**: `chore(submission-intake): pass quality checkpoint` (if fixes needed)
...
- [x] 8.6 [VERIFY] Quality checkpoint: build + all module tests pass
  - **Do**: `dotnet build src/BrokerConnect.slnx && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Domain.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.Api.Tests && dotnet test tests/Modules/SubmissionIntake/SubmissionIntake.IntegrationTests`
  - **Verify**: All exit 0
  - **Done when**: No errors
  - **Commit**: `chore(submission-intake): pass quality checkpoint` (if fixes needed)
