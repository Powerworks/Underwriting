...
- [X] T045 [P] [US2] Test `Inline` snapshot reflects a grant in the same request in `tests/Modules/AuthorityAdministration/AuthorityAdministration.IntegrationTests/AuthorityMatrixInlineSnapshotTests.cs` — grants via `GrantCellAuthorityLimitHandler` then reads it back through the actual `GetAuthorityMatrix` endpoint handler (T047) in the same session; plus a `NotFound` case for an unknown id. Passing.

### Implementation for User Story 2

- [X] T046 [US2] Register `Projections.Snapshot<AuthorityLimit>(SnapshotLifecycle.Inline)` — **consolidated into `Module.cs`** (same T006 pattern), not a separate DbConfig file. Registration compiled since early in the session but was never actually run against real Postgres until T010/T018/T026/T035's Testcontainers work in this session, which surfaced (and this task's fix resolved) two blocking bugs — see T029's note. Now verified working: a grant is queryable via `session.Query<AuthorityLimit>()`/`LoadAsync` in the same session.
- [X] T047 [US2] `GET /api/v1/authority/matrix/{authorityLimitId}` `[WolverineGet]` endpoint in `.../Api/ReadModels/AuthorityMatrix/GetAuthorityMatrix.cs` (depends on T046) — returns a dedicated `AuthorityMatrixResponse` DTO (constitution Principle VII: never the aggregate itself), not `AuthorityLimit` directly. **Gap flagged, not fixed**: data-model.md's `AuthorityMatrix` shape also lists `providerId` and `currency`; neither is persisted anywhere upstream (not on `AuthorityLimit`, not on either `*Granted` event — `GrantCellAuthorityLimitRequest.Currency` is validated but never stored). The response DTO exposes only what the aggregate actually carries; inventing values for the missing two would be worse than omitting them.
- [X] T048 [US2] Wire T016's and T025's duplicate/cascade checks to query this snapshot directly rather than re-deriving state — **already wired this way** when T016/T025/T033 were first written (each already calls `session.Query<AuthorityLimit>()`, per T016's own comment: "functionally live once T046 registers the Inline snapshot"); T046's bug fixes this session are what makes that query path actually correct rather than silently returning nothing. No handler code changes needed — verified via T019/T020/T027/T028's rejection-path tests, which exercise exactly this query.
...
- [X] T049 [P] [US6] Projection test — **retargeted to Layer 3** (same rationale as T010 etc.): `tests/Modules/AuthorityAdministration/AuthorityAdministration.IntegrationTests/CellAuthorityRegisterProjectionTests.cs`, not Api.Tests — this is an Async-lifecycle projection, so exercising it honestly requires the real Testcontainers Postgres + async daemon, not a mock. Covers grant→history entry, revoke→`supersededAt`, an Underwriter-tier revocation correctly *not* touching the Cell register it happens to route to, and T051's pagination. Passing.

### Implementation for User Story 6

- [X] T050 [P] [US6] `CellAuthorityRegister` async projection (`data-model.md` field list) reacting to `CellAuthorityLimitGranted`, `AuthorityLimitRevoked` in `.../Api/ReadModels/CellAuthorityRegister/CellAuthorityRegisterProjector.cs` — `MultiStreamProjection<CellAuthorityRegister, string>` keyed by `cellId` (declared `partial`, same source-generator requirement discovered for the Inline snapshot). **Two things beyond the literal task description, both flagged rather than silently done**: (1) `AuthorityLimitRevoked` needed a new `CellId` field — its own stream is keyed by `authorityLimitId`, and without `CellId` on the event itself a cross-stream projection has no way to route it correctly; `RevokeAuthorityLimitHandler` already had `entity.CellId` in scope. (2) The board's own dependency edges for this read model list only `CellAuthorityLimitGranted`/`AuthorityLimitRevoked` as sources, not `AuthorityLimitRevised` — confirmed against spec.md's Event Model Detail appendix (verbatim board export), not just data-model.md, so this is the literal spec and not a missing-edge bug like the earlier AuthorityMatrix fix. Implemented as specified: a grant later revised (not revoked) will not show that revision here. Also required wiring the async daemon, absent until now: `Program.cs` had `IntegrateWithWolverine()` but no `.AddAsyncDaemon(DaemonMode.Solo)` — without it no `Async`-lifecycle projection would ever actually run in production, not just in this feature. The test fixture needed the equivalent (`BuildProjectionDaemonAsync`/`StartAllAsync`/`WaitForNonStaleData`).
- [X] T051 [US6] `GET /api/v1/authority/cells/{cellId}/register` endpoint in `.../Api/ReadModels/CellAuthorityRegister/GetCellAuthorityRegister.cs` — paginate `history[]`. Page size TBD **flagged, not silently guessed**: default 20 / max 200, both named constants with a comment noting they're a guess, caller-overridable via `?pageSize`; history ordered newest-first by `effectiveDate`.
