...
> **Corrected 2026-08 against the live deployed API** (Pine Walk / Broker Connect
> board). The `contexts/:contextName/slices/:sliceName/...` paths below were
> previously documented here but **do not exist on the deployed server** —
> every variant 404s with Express's raw `Cannot GET/POST ...` (no route
> matched at all, not an app-level 404). Confirmed via the live
> `GET /swagger.json` on `api.eventmodelers.ai`, which lists only the
> `timelineId`/`columnId`-addressed routes below. Trust this section, not
> context/slice-name-based paths, for this API area.

### POST `/api/org/:orgId/boards/:boardId/timelines/:timelineId/columns/:columnId/scenarios`
Append one or more GWT scenarios to a column's spec node. `timelineId` is the
CHAPTER node's UUID; `columnId` is the specific column's UUID (**not** a
context/slice name — get both from `GET .../nodes/:chapterId`'s
`meta.timelineData.columns`, or from `learn-eventmodelers-api`'s own node
inspection). The SCENARIO spec node is created automatically if it doesn't
exist yet.

**Request body** — a single scenario object, or an array of them:
```typescript
{
  id: string       // required — arbitrary string, not validated as UUID
  title: string     // required — must be unique within the spec node
  given?: SpecStep[]
  when?: SpecStep[]
  then?: SpecStep[]
  vertical?: boolean
...
// SpecStep — confirmed by testing; a bare string nodeId is REJECTED
// ("an item in 'when' is missing the required 'id' field"):
{
  id: string        // required — the referenced COMMAND/EVENT/READMODEL node's id
  title?: string     // accepted; not confirmed whether it's validated against the node
  type?: string       // accepted; "COMMAND" | "EVENT" | "READMODEL" — same caveat
}
```

**Validation rules** (enforced before the request reaches business logic —
confirmed via real 400 responses):
- Top level: `id` and `title` are required strings, or you get
  `400 {"error":"Each scenario must have: id (string), title (string); optional: given[], when[], then[], vertical, examples"}`
- `given`: only EVENTs from the same timeline
...
**Response**:
- `201` — `{ specNodeId, scenarios: [...], added: number, isNewNode: boolean }`
- `400` — validation error (see above)
- `404` — `{"error":"Column '<id>' not found in timeline '<id>'"}` — bad `timelineId`/`columnId`
- `409` — duplicate scenario title
- `500` — `{"error":"Server error"}` — **can mean a genuinely corrupted column**,
  not just a transient failure. Confirmed on this board: one specific column
  (an original slice's first column, present since the very first bulk
  `import-config` call) 500s on *every* request to this endpoint — including
  a fully empty `{given:[],when:[],then:[]}` payload — while every other
  column on the same timeline, including brand-new cell-less ones, succeeds.
  Root cause not identified (no way to inspect server logs from here). If you
  hit a persistent 500 that doesn't respond to payload changes, don't keep
  guessing at the request shape — **test a different column on the same
  timeline first** to check whether the column itself is bad, and route
  around it (attach the scenario to a sibling column in the same slice/flow
  instead — cross-column references in `given`/`then` are legal as long as
  they're the same timeline).
- There is no delete-scenario endpoint. A scenario, once posted, cannot be
  removed via this API — get the payload right before sending.

---

### GET `/api/org/:orgId/boards/:boardId/timelines/:timelineId/spec-info`
Returns every EVENT, COMMAND, and READMODEL node whose `parentId` matches the
given timeline — i.e. every node legally referenceable in that timeline's
`given`/`when`/`then` steps.

**Response**: `200` — `{ timelineId: string, elements: [{ id, title, type }, ...] }`

(The `contexts/:contextName/...` forms of this endpoint documented in earlier
revisions of this file do not exist on the deployed API — see note above.
There is also no `MODEL_CONTEXT` node registry backing a context-name lookup:
`GET .../nodes?type=MODEL_CONTEXT` returns `[]` even on a board with many
named contexts, because bulk `import-config`'s per-element `modelContext`
field is not persisted onto the created nodes.)
